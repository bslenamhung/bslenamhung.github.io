...
function ensureData(){const base=cloneDefault();D={...base,...D,site:{...base.site,...(D.site||{})},clinic:{...base.clinic,...(D.clinic||{})},specialties:Array.isArray(D.specialties)?D.specialties:[],services:Array.isArray(D.services)?D.services:[],articles:Array.isArray(D.articles)?D.articles:[]};D.clinic.images=Array.isArray(D.clinic.images)?D.clinic.images:['','','',''];D.clinic.weeklyScheduleImage=D.clinic.weeklyScheduleImage||'';if(D.clinic.zalo==='http://zaloapp.com/qr/p/quocjkn8vcrk')D.clinic.zalo='https://zalo.me/84946444812';D.site.bioImage=D.site.bioImage||'';D.site.careerImage=D.site.careerImage||'';D.site.expertiseImage=D.site.expertiseImage||'';D.site.researchImage=D.site.researchImage||'';D.services=D.services.map(x=>typeof x==='string'?{name:x,desc:'',published:true}:({...x,name:x.name||'Dịch vụ mới',desc:x.desc||'',published:x.published!==false}));D.articles=D.articles.map(x=>({...x,id:x.id||articleId(x),seoTitle:x.seoTitle||'',seoDescription:x.seoDescription||'',keywords:x.keywords||'',seoImage:x.seoImage||'',slug:x.slug||''}));}
...
async function isAdmin(){try{const {data:{user},error:userError}=await client.auth.getUser();if(userError||!user)return {ok:false,error:userError?.message||'Không xác thực được tài khoản.'};const {data,error}=await client.from('admin_users').select('user_id').eq('user_id',user.id).maybeSingle();return {ok:!error&&!!data,error:error?.message||null,user}}catch(e){return {ok:false,error:e?.message||'Không xác thực được tài khoản.'}}}
async function boot(){if(!window.supabase||!window.SUPABASE_URL?.startsWith('http')){location.href='admin-login.html';return}client=window.supabase.createClient(window.SUPABASE_URL,window.SUPABASE_ANON_KEY);const {data:{session}}=await client.auth.getSession();if(!session){location.href='admin-login.html';return}const verified=await client.auth.getUser();if(verified.error||!verified.data?.user){await client.auth.signOut();location.href='admin-login.html';return}const guard=await isAdmin();if(!guard.ok){setStatus('Tài khoản này không có quyền quản trị.',false);await client.auth.signOut();setTimeout(()=>location.href='admin-login.html',1000);return}const {data,error}=await client.from('site_content').select('content').eq('id',1).maybeSingle();if(error){setStatus('Không đọc được dữ liệu: '+error.message,false);return}if(data?.content)D=data.content;ensureData();renderAll().catch(err=>setStatus('Lỗi tải quản trị: '+(err?.message||err),false))}
...
function openArticleEditor(index=-1){
  ...
  const a=index>=0?D.articles[index]:{title:"",slug:"",slug:"",slug:"",slug:"",slug:"",slug:"",slug:"",slug:"",slug:"",slug:"",slug:"",slug:"",slug:"",slug:"",slug:"",slug:"",slug:"",slug:"",slug:"",slug:"",slug:"",slug:"",slug:"",slug:"",slug:"",slug:"",slug:"",slug:"",slug:"",slug:"",slug:"",slug:"",slug:"",slug:"",slug:"",slug:"",slug:"",specialty:D.specialties[0]?.name||"Sản khoa",desc:"",image:"",content:"",videoUrl:"",published:false,seoTitle:"",seoDescription:"",keywords:"",seoImage:""};
...
function saveArticleFromModal(){
  ...
  const slug=($("editSlug")?.value||"").trim();
   const obj={id:current?.id||('art-'+Date.now().toString(36)+'-'+Math.random().toString(36).slice(2,8)),title,slug,specialty:$("editSpecialty").value||D.specialties[0]?.name||"Sản khoa",desc:$("editDesc").value.trim(),image:cover,content:$("editContent").innerHTML.trim(),published,seoTitle:$("editSeoTitle").value.trim()||seoDefaultsFromCurrent().seoTitle,seoDescription:$("editSeoDescription").value.trim()||seoDefaultsFromCurrent().seoDescription,keywords:$("editKeywords").value.trim()||seoDefaultsFromCurrent().keywords,seoImage:$("editSeoImage").value.trim()||cover,updatedAt:now,createdAt,publishedAt};
...
async function purgeLegacyArticles(){
  ensureData();
  const legacy=D.articles.filter(a=>articleId(a).startsWith('legacy-'));
  if(!legacy.length){setStatus('Không có bài viết cũ dạng legacy cần xóa.');return;}
  const first=confirm(`Có ${legacy.length} bài viết cũ chưa có ID chuẩn. Xóa các bài này để tạo lại bài mới?`);
  if(!first)return;
  const second=confirm('Xác nhận lần cuối: các bài cũ dạng legacy sẽ bị xóa khỏi dữ liệu online. Sau đó anh có thể tạo bài mới.');
  if(!second)return;
  D.articles=D.articles.filter(a=>!articleId(a).startsWith('legacy-'));
  renderAll();
  await save();
  setStatus(`✅ Đã xóa ${legacy.length} bài cũ. Bây giờ hãy tạo bài viết mới.`);
}
