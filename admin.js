...
async function uploadPublicImage(file,prefix){
  ...
  const guard=await isAdmin(); if(!guard.ok) throw new Error('Phiên quản trị không hợp lệ.');
  const ext=(file.name.split('.').pop()||'jpg').toLowerCase().replace(/[^a-z0-9]/g,'')||'jpg';
  const safePrefix=prefix.replace(/[^a-z0-9_-]/gi,'-');
  const path=`articles/${safePrefix}-${Date.now()}-${Math.random().toString(36).slice(2,8)}.${ext}`;
  const {error}=await client.storage.from('site-media').upload(path,file,{upsert:false,cacheControl:'31536000',contentType:file.type});
