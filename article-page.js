...
async function recordArticleViewPage(a){
  try{
    if(window.__BSHUNG_ARTICLE_VIEW_RECORDED__) return;
    const id=articleIdPage(a); if(!id || !window.supabase || !window.SUPABASE_URL) return;
    const c=window.supabase.createClient(window.SUPABASE_URL,window.SUPABASE_PUBLISHABLE_KEY||window.SUPABASE_ANON_KEY);
    const r=await c.rpc('record_article_view',{p_article_id:id,p_title:String(a?.title||'')});
    if(r?.error) throw r.error;
    window.__BSHUNG_ARTICLE_VIEW_RECORDED__=true;
