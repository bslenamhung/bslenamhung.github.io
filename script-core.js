...
async function getData(){try{const u=window.SUPABASE_URL||'https://tufzlxfrtdhestbemcly.supabase.co';const k=window.SUPABASE_PUBLISHABLE_KEY||window.SUPABASE_ANON_KEY;if(!k)return null;const r=await fetch(u+'/rest/v1/site_content_public?id=eq.1&select=content',{headers:{apikey:k,Authorization:'Bearer '+k}});if(!r.ok)return null;const rows=await r.json();return rows&&rows[0]&&rows[0].content?rows[0].content:null;}catch(e){console.error('Supabase:',e);return null}}
function bindAbout(){
...
function showAbout(title,text){
 ...
 if(!modal){modal=document.createElement('div');modal.id='doctorAboutModal';modal.setAttribute('role','dialog');modal.setAttribute('aria-modal','true');modal.style.cssText='position:fixed;inset:0;z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px';modal.innerHTML='<div class="doctor-about-backdrop" style="position:absolute;inset:0;background:rgba(0,0,0,.5)"></div><div class="doctor-about-dialog" style="position:relative;max-width:720px;width:100%;max-height:85vh;overflow:auto;background:#fff;border-radius:20px;padding:30px;box-shadow:0 20px 60px rgba(0,0,0,.25)"><button type="button" class="doctor-about-close" aria-label="Đóng" style="position:absolute;right:14px;top:10px;border:0;background:none;font-size:30px;cursor:pointer">×</button><p class="eyebrow">BS LÊ NAM HÙNG</p><h2 id="doctorAboutTitle"></h2><div id="doctorAboutBody" class="doctor-about-body" style="white-space:pre-line;line-height:1.8"></div></div>';document.body.appendChild(modal);modal.querySelector('.doctor-about-backdrop').onclick=()=>modal.remove();modal.querySelector('.doctor-about-close').onclick=()=>modal.remove();}
