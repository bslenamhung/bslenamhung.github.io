-- ============================================================
-- BS LÊ NAM HÙNG - SECURITY HARDENING V45.2
-- Bản sửa lỗi Storage: KHÔNG ALTER/GRANT/REVOKE storage schema.
-- Chỉ dùng RLS policies cho storage.objects theo khuyến nghị Supabase.
-- Chạy sau khi đã có auth user quản trị trong public.admin_users.
...
-- ============================================================
-- 3) PUBLIC CONTENT: chỉ đọc, loại bài nháp
-- ============================================================
    ...
    coalesce(
      (
        ...
        from jsonb_array_elements(
          coalesce(new.content->'articles','[]'::jsonb)
...
for each row
execute function private.sync_site_content_public();
  ...
  jsonb_set(
    ...
    coalesce(
      (
        select jsonb_agg(x.value order by x.ordinality)
        from jsonb_array_elements(
          coalesce(s.content->'articles','[]'::jsonb)
        ) with ordinality as x(value, ordinality)
        where coalesce(x.value->>'published','true') <> 'false'
      ),
      '[]'::jsonb
...
-- ============================================================
-- 5) STORAGE
-- KHÔNG ALTER/GRANT/REVOKE trên storage.objects.
-- Supabase coi storage schema là read-only; chỉ tạo RLS policies.
-- Bucket site-media vẫn public để khách xem ảnh.
-- ============================================================
...
-- Chỉ Admin mới được xem metadata qua API Storage
...
-- Chỉ Admin mới được update/ghi đè file
...
-- ============================================================
-- 6) LƯỢT XEM BÀI: chỉ ghi qua RPC và chỉ bài đã xuất bản
-- ============================================================
...
begin
  ...
  insert into public.article_view_stats(
    article_id, title, view_count, updated_at
  )
  values (
    p_article_id,
    pg_catalog.coalesce(p_title, ''),
    1,
    pg_catalog.now()
  )
  on conflict (article_id) do update
    set view_count = public.article_view_stats.view_count + 1,
        title = case
          when pg_catalog.btrim(pg_catalog.coalesce(excluded.title, '')) <> ''
          then excluded.title
          else public.article_view_stats.title
        end,
...
declare
  ...
  v_month text := pg_catalog.to_char(
...
begin
  insert into public.site_visit_stats(
    id, total_visits, today_visits, month_visits, today_key, month_key
  )
...
-- 8) KIỂM TRA
-- ============================================================
select 'admin_users' as check_name, count(*) as rows
from public.admin_users;

select 'site_content' as check_name, count(*) as rows
from public.site_content;

select 'site_content_public' as check_name, count(*) as rows
from public.site_content_public;

select 'published_articles_in_public' as check_name,
       pg_catalog.jsonb_array_length(
         coalesce(content->'articles','[]'::jsonb)
       ) as rows
from public.site_content_public
where id = 1;
