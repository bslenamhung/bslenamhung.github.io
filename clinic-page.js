...
async function loadClinic(){
  ...
  try{const c=window.supabase?.createClient?.(window.SUPABASE_URL,window.SUPABASE_PUBLISHABLE_KEY||window.SUPABASE_ANON_KEY);if(c){const r=await c.from('site_content').select('content').eq('id',1).maybeSingle();if(!r.error&&r.data?.content)data=r.data.content;}}catch(e){}
