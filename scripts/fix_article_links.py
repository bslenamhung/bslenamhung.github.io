...
def main():
    ...
    for p in Path('bai-viet').glob('*.html'):
        text=p.read_text(encoding='utf-8'); m=re.search(r'data-article-id=["\']([^"\']+)',text,re.I)
        if m: id_to_file[m.group(1)]=p.name
    changed=0
    ...
    for p in [x for x in Path('.').rglob('*.html') if '.git' not in x.parts]:
        text=p.read_text(encoding='utf-8'); original=text; base='../' if p.parent.name=='bai-viet' else ''
        def target(prefix,filename):
            if prefix.lower().startswith(('http://','https://')) or prefix.startswith('//'): return 'https://bslenamhung.github.io/bai-viet/'+filename
        ...
        if text!=original: p.write_text(text,encoding='utf-8'); changed+=1
